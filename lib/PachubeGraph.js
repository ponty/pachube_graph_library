...
//todo
    var x= new Date(self.data[0][length - 1][0]);
    ...
    , callback: function(irc_par){ return function(result) {
        for (var i=0; i < result.datapoints.length; i++) {
          ...
          if (point_at > last_received_at) {
            self.data[irc_par].push([point_at, parseFloat(point.value)]);
          }
        }
  ...
  self.websocketUpdate = function() {
    ...
    , callback: function(irc_par){ return function(message) {
        ...
        if (point_at > last_received_at) {
          self.data[irc_par].push([point_at, parseFloat(point.current_value)]);
        }
  ...
  self.draw = function() {
    ...
    var plot = $.plot( self.canvas
          ...
          , { xaxis: {
                        mode: "time"
                     , min: new Date(self.start.valueOf())
                     , max: new Date(self.end.valueOf())
                     }
            , yaxis: {
