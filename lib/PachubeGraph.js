...
var tz=-60000*(new Date().getTimezoneOffset())
...
function PachubeGraph(element) {
  ...
  self.init = function(element) {
    ...
    self.settings = {
      resource: self.element.attr('pachube-resource').split(" ")
    ...
    self.data = new Array();
    for (irc in self.settings.resource)
        self.data[irc] = new Array();
    ...
    if (self.settings.update) {
      ...
      $(document).ready(function() {
        setInterval(function() { self.draw(); }, 30000);
      });
    }
  };
  ...
  self.parse_pachube_options = function() {
    ...
    self.settings.lines = !(options_hash["hide-lines"] == "true");
    self.settings.points = !(options_hash["hide-points"] == "true");
    self.settings.hoverable = true;
    self.settings.legend = (options_hash["legend"] == "true");
    self.settings.yAxisMinTickSize = (options_hash["yAxisMinTickSize"] || null);
  ...
  self.set_timespan_and_interval = function(str) {
    switch(str) {
      case "3 months":
        ...
        break;
      case "1 month":
          self.settings.timespan = 31*86400000;
          self.settings.interval = 31*900;
          break;
      case "2 weeks":
          self.settings.timespan = 14*86400000;
          self.settings.interval = 14*900;
          break;
      case "1 week":
          self.settings.timespan = 7*86400000;
          self.settings.interval = 7*900;
          break;
      case "4 days":
...
//todo
    var x= new Date(self.data[0][length - 1][0] - tz);
  ...
  self.update = function(callback) {
    ...
    pachubeAPI.datastreamGet({
      resource: self.settings.resource[irc]
    ...
    , find_previous: true
    , per_page: 2000
    , callback: function(irc_par){ return function(result) {
        for (var i=0; i < result.datapoints.length; i++) {
          ...
          if (point_at > last_received_at) {
            self.data[irc_par].push([point_at+tz, parseFloat(point.value)]);
          }
        }
  ...
  self.websocketUpdate = function() {
    ...
    , interval: self.settings.interval
    , per_page: 2000
    , callback: function(irc_par){ return function(message) {
        var point = JSON.parse(message.data).body;
        var point_at = Date.parse(point.at.substring(0,23) + "Z");
        if (point_at > last_received_at) {
          self.data[irc_par].push([point_at+tz, parseFloat(point.current_value)]);
        }
  ...
  var timespan_link = function(value) {
    ...
    link.onclick = function() {
      ...
      for (irc in self.settings.resource)
        self.data[irc] = []; // get rid of the data we already got
      self.update();
      return false;
    };
    return link;
  ...
  self.draw = function() {
    ...
    if (self.link_bar == undefined) {
      ...
      link_bar.appendChild(timespan_link("1 month"));
      link_bar.appendChild(document.createTextNode(' | '));
      link_bar.appendChild(timespan_link("2 weeks"));
      link_bar.appendChild(document.createTextNode(' | '));
      link_bar.appendChild(timespan_link("1 week"));
      link_bar.appendChild(document.createTextNode(' | '));
    ...
    if (self.logo == undefined) {
      var link = document.createElement('a');
      link.href = "http://www.xively.com";
      link.innerHTML = 'powered by www.xively.com';
    ...
    a=[];
    for(irc in self.data)
    {
        a.push({
            label: self.settings.resource[irc].split('/').slice(-1)[0], // .slice(-1)[0] => last
               data: self.data[irc]
              });
    }
    var plot = $.plot( self.canvas
          , a
          , { xaxis: {
                        mode: "time"
                     , min: new Date(self.start.valueOf()+tz)
                     , max: new Date(self.end.valueOf()+tz)
                     }
            , yaxis: {
                      minTickSize: self.settings.yAxisMinTickSize
                     }
            , grid:  { color: self.settings.textColor
                     ...
                     , hoverable: self.settings.hoverable
                     , clickable: false
                     }
            , series: {
                  lines: { show: self.settings.lines },
                  points: { show: self.settings.points }
                      }
			, legend: { show: self.settings.legend
                      , position: "nw"
			          }
            }
          );
