function PachubeAPI(options) {
  ...
  self.init = function() {
    ...
    self.settings.api_url = 'http://api.xively.com/v2/';
  ...
  self.setupWebSocket = function() {
    try {
      if ("WebSocket" in window) {
        ...
        var ws = new WebSocket("ws://beta.xively.com:8080/");
  ...
  self.datastreamGet = function(options) {
    // add '/' if missing
    if (options.resource.charAt(0) != '/')
	    options.resource = '/' + options.resource
	// add /feeds if missing    
	if (options.resource.substr(0,6) != '/feeds')
	    options.resource = '/feeds' + options.resource
  ...
  self.subscribe = function(options) {
    ...
    try {
      self.websocket.send('{"method": "subscribe", "resource": "'+options.resource.substr(6)+'", "headers":{"X-PachubeApiKey":"'+(options.api_key || self.settings.api_key)+'"}');
